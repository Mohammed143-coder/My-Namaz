  ...
  return (
    <div className="bg-white min-h-screen text-black p-1 overflow-y-auto mb-4">
      ...
      <div className="md:flex mb-2">
        <div className="mt-4 p-2 md:w-[50%]">
              ...
              <div key={index} className="space-y-2">
                {Object.entries(item.namazTiming || {}).map(
                  ([prayer, details]) => (
                    <div
                      key={prayer}
                      className="flex items-center justify-between border border-gray-400 rounded-lg p-2 shadow-sm"
                    >
        ...
        <div className="my-2 p-2 mb-8 md:w-[50%] md:mt-4">
          <p className="font-semibold text-base mb-3">Masjid's Announcements</p>
          {announcement?.length > 0 ? (
            announcement?.map((item, index) => (
              <div
                className="border border-gray-400 rounded-lg shadow-sm text-center mt-2 p-1"
                key={index}
              >
                <p>{item.message}</p>
                <small>
